...
exp_a = 0.8;
thr_e = 0.01;
w0 = gpuArray(single(w0));
exp_a = gpuArray(single(exp_a));
thr_e = gpuArray(single(thr_e));
...
p_outlier = gpuArray(single(p_outlier));
p_b_given_outlier = gpuArray(single(p_b_given_outlier));
...
blurred_w = wrap_boundary_liu0(blurred, opt_fft_size0([H W]+size(psf)-1));

% create the initial mask
mask = zeros(size(blurred_w));
...
if nargout == 2
    mask = s(1:size(blurred,1), 1:size(blurred,2));
end
