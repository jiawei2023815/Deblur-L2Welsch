function ret = wrap_boundary_liu0(img, img_size)
% wrap_boundary_liu.m
%
...
% Gx Gy and boundary image should be of same size
    ...
    % Laplacian
    f = zeros(H,W); f = gpuArray(single(f));
    clear j k

    % boundary image contains image intensities at boundaries
    boundary_image(2:end-1, 2:end-1) = 0;
    j = 2:H-1;      k = 2:W-1;      f_bp = zeros(H,W); f_bp = gpuArray(single(f_bp));
