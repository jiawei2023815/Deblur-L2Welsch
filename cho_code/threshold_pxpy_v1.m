...
psf_size = gpuArray(psf_size);

if ~exist('threshold', 'var')
    threshold = 0;
    threshold = gpuArray(single(threshold));
    b_estimate_threshold = true;
else
    b_estimate_threshold = false;
end

b_estimate_threshold = gpuArray(single(b_estimate_threshold));
...
%%
% derivative filters
dx = [-1 1; 0 0]; dx = gpuArray(single(dx));
dy = [-1 0; 1 0]; dy = gpuArray(single(dy));
%%
